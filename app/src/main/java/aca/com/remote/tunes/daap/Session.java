...
public class Session {
	...
	protected void fireAction(final String url, final boolean notify, final ConnectionResponseListener listener) {
		ThreadExecutor.runTask(new Runnable() {
			public void run() {
				try {
					RequestHelper.request(url,false,listener);
					if (notify)
						notifyStatus();
				} catch (Exception e) {
					Log.e(TAG, "Fire Action Exception:" + e.getMessage());
	...
	public void setRadioTunesUrl(String url, String type, String artwork) {
		...
		String request_url;
		if(null == artwork || artwork.isEmpty()){
            request_url = String.format("%s/ctrl-int/1/seturl?url=%s&type=%s&com.apple.itunes.extended-media-kind=1"+ "&session-id=%s",
                    this.getRequestBase(), url, type, this.sessionId);
        } else {
            request_url = String.format("%s/ctrl-int/1/seturl?url=%s&type=%s&artwork=%s&com.apple.itunes.extended-media-kind=1"+ "&session-id=%s",
                    this.getRequestBase(), url, type, artwork, this.sessionId);
        }
		this.fireAction(request_url, data, false);
	}
	...
	public void httpserver(String url) {
		...
		this.fireAction(String.format("%s/ctrl-int/1/setproperty?mymusic-url=%s&com.apple.itunes.extended-media-kind=1"+ "&session-id=%s",
	...
	public void configWifi(String ssid, String pwd) {
		Log.w(TAG, String.format("wifi config, SSID:%s , password:%s ", ssid,pwd));
		this.fireAction(String.format("%s/wificfg?ssid=%s&password=%s", this.getRequestBase(), ssid,pwd), false);
	}

	public void configWifiStateQuery(final ConnectionResponseListener listener) {
        //ConnectionResponseListener listener = new ConnectionResponseListener{};
		this.fireAction(String.format("%s/wificfg?state=query", this.getRequestBase()), false,listener);
	}

	/*speak's special play cmd for music services Spotify,SHOUTcast,Bluetooth,DLNA*/
	public void onlinePlay(String service) {
		// http://192.168.254.128:3689/service-start?service=Spotify
		this.fireAction(String.format("%s/service-start?service=%s", this.getRequestBase(), service), false);
	}
