...
public class Status {
	...
	public final static int UPDATE_SPEAKERS = 7;
    public final static int UPDATE_WIFIAPS = 8;
	public final static int UPDATE_WIFIAPS_M = 9;
		...
		public void run() {
			while (true) {
				try {
					...
					Log.d(TAG, String.format("Exceptio7n in keepalive thread, so killing try# %d", failures.get()), e);
	...
	protected void parseUpdate(Response resp) throws Exception {
		...
		if (trackId != this.trackId || containerItemId != this.containerItemId || playlistId != this.playlistId
				|| databaseId != this.databaseId || trackName != this.trackName) {
			...
			ThreadExecutor.runTask(new Runnable() {
				public void run() {
					try {
						...
						coverCache = RequestHelper.requestBitmap(String.format("%s/ctrl-int/1/nowplayingartwork?mw="
								+ screenHeight + "&mh=" + screenHeight + "&session-id=%s", session.getRequestBase(),
								session.sessionId));
					} catch (Exception e) {
	...
	public List<WifiAp> getWifiAps(final List<WifiAp> wifiAps) {
		try {
			Log.d(TAG, "getWifiAps() requesting...");

			String temp = String.format("%s/wificfg?list=get", session.getRequestBase());
			...
			for (Response mdcl : mdclArray) {
				...
				wifiAp.setSsid(mdcl.getString("minm").trim());

				Log.d(TAG, "wifiAp = " + wifiAp.getSsid());
                if(null != wifiAp.getSsid() &&  ( ! "".equals( wifiAp.getSsid())))
				    wifiAps.add(wifiAp);
			}

		} catch (Exception e) {
			Log.e(TAG, "Could not get wifi AP list: ", e);
			wifiAps.clear();
		}
		return wifiAps;
	}
